    ...
    if current_room_monster:
        if 'Armored Skeleton' in current_room_monster:
            if not current_room_monster['Armored Skeleton'].get_defeated_status():
                print "\n The Armored Skeleton attacks!"
                if 'Sword' in character.get_inventory() and 'Bow' in character.get_inventory():
            ...
            if not current_room_monster['Animated Armor'].get_defeated_status():
                print "\n The Animated Armor attacks!"
                if 'Sword' in character.get_inventory() and 'Helmet' in character.get_inventory() and 'Shield' in character.get_inventory():
                    ...
                    print " by coming unprepared. The Armor reaches down and grabs you with its metal gauntlets, crushing your underprotected body.\n"
                    time.sleep(3)
                    i = 0
                    while i < 5:
                        print "."
            ...
            if not current_room_monster['Lich'].get_defeated_status():
                print "\n The Lich attacks!"
                time.sleep(2)
                if 'Sword' in character.get_inventory() and 'Armor Suit' in character.get_inventory():
